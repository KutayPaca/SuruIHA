...
# Hedefler: Karşıdan bakıldığında V formasyonu (x-z düzleminde)
# Ortadaki drone daha ileride ve daha alçakta, yanlar geride ve yüksekte
# Her drone kendi başlangıcına göre relative hedef alacak
relative_targets = [
    [ 1.0,  2.0,  3.0],# Sol kanat: ileri (y), sola (x), yüksekte (z)
    [ 2.0,  3.5,  5.0],  # Orta (lider): geride ve daha alçakta
      #en sol
    [3.0,  2.0,  5.0]  # Sağ kanat: ileri (y), sağa (x), yüksekte (z)
]
...
class DroneNode:
    ...
    def communicate(self, all_drones):
        return [d.position for d in all_drones if d.id != self.id]
...
for frame in range(10000):
    ...
    for drone in drones:
        ...
        print(f"Drone {drone.id} → Pozisyon: {np.round(drone.position, 3)} | Hedef: {np.round(drone.target, 3)}")

    if all(d.reached for d in drones):
        print("\n🎯 Tüm dronelar hedefe başarıyla ulaştı.")
        break
